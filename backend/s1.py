...
def image_to_base64(image):
    ...
    buffered = BytesIO()
    image.save(buffered, format="PNG")  # Save as PNG, can change format as needed
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        user_image = image_to_base64(img)
...
async def scrape_bumble_async(page):
    ...
    try:
        ...
        await facebook_button.click(force=False)  # Force click to bypass any potential issues with visibility
    ...
    try:
        image_sources = list()
        for _ in range(3):
            await asyncio.sleep(20)  # Wait for the page to load fully
            html_content = await page.content()
            soup = BeautifulSoup(html_content, 'html.parser')
            print(soup)
            image_tags = soup.find_all('img', class_='media-box__picture-image')
            print(image_tags)
            image_source = [img['src'] for img in image_tags if 'src' in img.attrs]
            image_sources.extend(image_source[:2])
            page.get_by_label("Pass").click()
            print(f"Scraped {len(image_sources)} images.")

        return image_sources # Ensure you return exactly 2 images
...
# async def scrape_images(page):
#     try:
#         image_sources = list()
#         for _ in range(1):  # you can extend this loop to scrape more pages if needed
...
#             # Find all image tags with the specific class
#             image_tags = soup.find_all('img', class_='media-box__picture-image')
            
#             # Extract 'src' attribute if it exists
#             image_source = [img['src'] for img in image_tags if 'src' in img.attrs]

#             # Extend the list with the first 2 images
#             image_sources.extend(image_source[:2])

#             # Log the number of images scraped
#             print(f"Scraped {len(image_sources)} images.")

#         # Return exactly two image URLs, if available
#         return image_sources[:2]  # Ensure only 2 images are returned
...
async def scrape_images():
    ...
    #         print('çomparing face')
    #         scores.append(face_comparer.compare_faces(user_image, base64_str))
    #         print(scores)
    #     else:
    ...
    scores = list()  # Dictionary to store image URL and score

# Your loop to process images
    for url in images:
        base64_str = image_url_to_base64(url)  # Convert image URL to base64
        print('\n\n\n\n going in with image \n\n\n\n')
        if base64_str:  # Only proceed if the base64 conversion is successful
            print(f"Base64 string length for {url}: {len(base64_str)}")
            
            # You may add "data:image/png;base64," as prefix, if required
            base64_images.append(f"data:image/png;base64,{base64_str}")
            
            print('Comparing face...')
            
            # Get the similarity score for the current image
            score = face_comparer.compare_faces(user_image, base64_str)
            
            # Store the score in the dictionary with the URL as the key
            scores.append(score)
            
            print(f"Scores so far: {scores[-1]}")
    ...
    # Return the response with both base64 images and their scores
    return jsonify({
        "message": "Scraping started",
        "images": base64_images,
        "image_scores": scores  # Include the image URL and score mapping
    }), 200
...
#     """Convert the image to a Base64 string"""
#     buffered = BytesIO()
#     image.save(buffered, format="PNG")  # Save as PNG, but you can change format as needed
#     return base64.b64encode(buffered.getvalue()).decode('utf-8')
...
#     base64_images = []
#     score = list()
#     # face_comparer = FaceComparer(method=method)
...
#             score.append(face_comparer.compare_faces.compare_facess(user_image, base64_str))
#             print('score', score)
...
#     # Return response with base64 images
#     return jsonify({"message": "Scraping started", "images": base64_images, "score": score}), 200
